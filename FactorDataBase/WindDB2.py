...
class _CalendarTable(_DBTable):
    ...
    def getID(self, ifactor_name=None, idt=None, args={}):
        ...
        return tuple(zip(*self.FactorDB.fetchall(SQLStr)))[0]
    # 返回交易所为 iid 的交易日列表
    ...
    def getDateTime(self, ifactor_name=None, iid=None, start_dt=None, end_dt=None, args={}):
        ...
        return tuple(map(lambda x: dt.datetime(int(x[0][:4]), int(x[0][4:6]), int(x[0][6:8]), 23, 59, 59, 999999), self.FactorDB.fetchall(SQLStr)))
...
class _ConstituentTable(_DBTable):
    ...
    def FactorNames(self):
        if self._IndexIDs is None:
            ...
            self._IndexIDs = tuple(zip(*self.FactorDB.fetchall(SQLStr)))[0]
    ...
    def getDateTime(self, ifactor_name=None, iid=None, start_dt=None, end_dt=None, args={}):
        ...
        return tuple(getDateTimeSeries(start_dt=start_dt, end_dt=end_dt, timedelta=dt.timedelta(1)))
    def _getRawData(self, fields, ids=None, start_date=None, end_date=None, args={}):
        DBTableName = self.FactorDB.TablePrefix+self.FactorDB.TableName2DBTableName([self.Name])[self.Name]
        FieldDict = self.FactorDB.FieldName2DBFieldName(table=self.Name, fields=['成份股Wind代码', '指数Wind代码', '纳入日期', '剔除日期', '最新标志'])
...
class _ETFPCFTable(WindMarketTable):
    ...
    def getID(self, ifactor_name=None, idt=None, args={}):
        ...
        SQLStr = "SELECT DISTINCT "+DBTableName+"."+FieldDict["成份股Wind代码"]+" "# ID
        SQLStr += "FROM "+DBTableName+" "
        SQLStr += "WHERE "+DBTableName+"."+FieldDict["基金Wind代码"]+"='"+args.get("基金ID", self._SysArgs["基金ID"])+"' "
        if idt is not None:
...
class _FeatureTable(_DBTable):
    ...
    def getID(self, ifactor_name=None, idt=None, args={}):
        ...
        return np.array(self.FactorDB.fetchall(SQLStr)).squeeze()
    def getDateTime(self, ifactor_name=None, iid=None, start_dt=None, end_dt=None, args={}):
        return np.array([], dtype=np.dtype("O"))
     # 时间点默认是当天, ID 默认是 [000001.SH], 特别参数: 回溯天数
...
class WindDB2(WindDB):
    ...
    def getTradeDay(self, start_date=None, end_date=None, exchange="上海证券交易所"):
        ...
        return np.array(list(map(lambda x: dt.date(int(x[0][:4]), int(x[0][4:6]), int(x[0][6:8])), self.fetchall(SQLStr))))
    ...
    def getID(self, index_id="全体A股", date=None, is_current=True):
        ...
        return np.array([iRslt[0] for iRslt in self.fetchall(SQLStr.format(Prefix=self.TablePrefix))])
    # --------------------------------------------信息转换-----------------------------------
    # 获取行业名称和Wind内部查询代码

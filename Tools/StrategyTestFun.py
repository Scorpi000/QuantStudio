...
def calcCLModel(wealth_seq, market_wealth_seq, risk_free_rate=0.0):
    ...
    return Rslt.params
# 保存策略到策略库
def saveStrategy(sub_lib,strategy_name,dss,cash_account,long_account,arbitrage_account,short_account,signal_merger,signal_prc,result,developer="",creation_date=None,strategy_info="",qs_env=None):
    AllStrategyNames = os.listdir(qs_env.SysArgs['StrategyLibPath']+os.sep+sub_lib)# 当前库中的所有策略名
    if (strategy_name in AllStrategyNames):
        qs_env.SysArgs['LastErrorMsg'] = '策略库中有重名!'
        return (0,AllStrategyNames)
    elif strategy_name=='':
        qs_env.SysArgs['LastErrorMsg'] = '策略名不能为空!'
        return (-1,AllStrategyNames)
    StrategyPath = qs_env.SysArgs['StrategyLibPath']+os.sep+sub_lib+os.sep+strategy_name
    os.mkdir(StrategyPath)
    with shelve.open(StrategyPath+os.sep+"StrategyInfo") as File:
        File['StrategyName'] = strategy_name
        File['Developer'] = developer
        if creation_date is None:
            creation_date = getCurrentDateStr()
            creation_date = creation_date[:4]+"-"+creation_date[4:6]+"-"+creation_date[6:]
        File['CreationDate'] = creation_date
        File['StrategyInfo'] = strategy_info
    for iDSName in dss:
        with shelve.open(StrategyPath+os.sep+"DS-"+iDSName) as File:
            dss[iDSName].saveInfo(File)
    with shelve.open(StrategyPath+os.sep+"CashAccount") as File:
        cash_account.saveInfo(File)
    with shelve.open(StrategyPath+os.sep+"LongAccount") as File:
        long_account.saveInfo(File)
    with shelve.open(StrategyPath+os.sep+"ArbitrageAccount") as File:
        arbitrage_account.saveInfo(File)
    with shelve.open(StrategyPath+os.sep+"ShortAccount") as File:
        short_account.saveInfo(File)
    with shelve.open(StrategyPath+os.sep+"SignalMerger") as File:
        signal_merger.saveInfo(File)
    for i,iSG in enumerate(signal_merger.SGList):
        iSGName = signal_merger.SGNameList[i]
        with shelve.open(StrategyPath+os.sep+iSGName.split('-')[0]+'-'+str(i)) as File:
            iSG.saveInfo(File)
    with shelve.open(StrategyPath+os.sep+"SignalProcessor") as File:
        signal_prc.saveInfo(File)
    with shelve.open(StrategyPath+os.sep+"Result") as File:
        result.saveInfo(File)
    AllStrategyNames.append(strategy_name)
    AllStrategyNames.sort()
    return (1,AllStrategyNames)

# 测试策略
def testStrategy(dates,dss,cash_account,long_account,arbitrage_account,short_account,signal_merger,signal_prc,result,qs_env=None,print_file=sys.stdout,prog_queue=None,gc_freq=22):
    import gc
    ...
    # 初始化
    print("策略回测初始化...",file=print_file)
    for jDS in dss:
        dss[jDS].start()
    cash_account.start()
    long_account.start()
    arbitrage_account.start()
    short_account.start()
    result.start()
    Error,Msg = signal_merger.start()
    if Error==-1:
        qs_env.SysArgs["LastErrorMsg"] = Msg
        for jDS in dss:
            dss[jDS].endDS()
        return (-1,None)
    for jSignalGen in signal_merger.SGList:
        Error,Msg = jSignalGen.start()
        if Error==-1:
            qs_env.SysArgs["LastErrorMsg"] = Msg
            for jDS in dss:
                dss[jDS].endDS()
            return (-1,None)
    signal_prc.start()
    print('策略回测初始化完成, 耗时 : %.2f' % (time.clock()-TotalStartT),file=print_file)
    # 策略回测
    print('策略回测运算中...',file=print_file)
    StartT = time.clock()
    iStartT = StartT
    #SignalGenTime = 0.0# debug
    #SignalPrcTime = 0.0# debug
    #OtherTime = 0.0# debug
    if PrintInfo:
        from progressbar import ProgressBar
        ProgBar = ProgressBar(max_value=len(dates))
        ProgBar.start()
    for i,iDate in enumerate(dates):
        for jDS in dss:
            dss[jDS].MoveOn(iDate)
        cash_account.MoveOn(iDate)
        long_account.MoveOn(iDate)
        arbitrage_account.MoveOn(iDate)
        short_account.MoveOn(iDate)
        #OtherTime += time.clock()-iStartT# debug
        #iStartT = time.clock()# debug
        iSignal = signal_merger.genSignal(iDate)
        #SignalGenTime += time.clock()-iStartT# debug
        #iStartT = time.clock()# debug
        signal_prc.processSignal(iSignal,iDate)
        #SignalPrcTime += time.clock()-iStartT# debug
        #iStartT = time.clock()# debug
        prog_queue.put(i+1)
        if PrintInfo:
            ProgBar.update(i+1)
    ...
    if PrintInfo:
        ProgBar.finish()
    print('策略回测运算完成, 耗时 : %.2f' % (time.clock()-StartT),file=print_file)
    #print("其他耗时 : %.2f" % (OtherTime,),file=print_file)# debug
    #print("信号生成耗时 : %.2f" % (SignalGenTime,),file=print_file)# debug
    #print("信号处理耗时 : %.2f" % (SignalPrcTime,),file=print_file)# debug
    # 生成结果
    print('策略结果生成中...',file=print_file)
    StartT = time.clock()
    Output = {}
    for jDS in dss:
        Output.update(dss[jDS].endDS())
    Output.update(signal_merger.endSG())
    for jSG in signal_merger.SGList:
        Output.update(jSG.endSG())
    Output.update(signal_prc.endSP())
    Output.update(short_account.endAccount())
    Output.update(arbitrage_account.endAccount())
    Output.update(long_account.endAccount())
    Output.update(cash_account.endAccount())
    result.genPrimaryResult()
    Output.update(result.genReport())
    EndT = time.clock()
    print('策略结果生成结束, 耗时 : %.2f' % (EndT-StartT),file=print_file)
    print('策略回测完成, 总运行时间 : %.2f' % (EndT-TotalStartT),file=print_file)
    return (1,Output)
# 加载策略库中的策略
def loadLibStrategy(sub_lib,strategy_name,qs_env):
    import DataSource
    import Account
    import SGClasses
    import TimingSGClasses
    import SMClasses
    import SPClasses
    import Report
    StrategyPath = qs_env.SysArgs['StrategyLibPath']+os.sep+sub_lib+os.sep+strategy_name
    # 加载数据源
    AllDSs = {}
    AllFiles = listDirFile(StrategyPath,suffix='dat')
    for iFileName in AllFiles:
        if iFileName[:3]=='DS-':
            iDSName = iFileName[3:]
            with shelve.open(StrategyPath+os.sep+iFileName) as File:
                iDSType = File['DSType']
                iFactorDBType = File['FactorDBType']
                iFactorDB = getattr(qs_env,iFactorDBType,qs_env.FactorDB)
                iDS = DataSource.DSClasses[iDSType](iDSName, iFactorDB, qs_env)
                Error = iDS.loadInfo(File)
            if Error!=1:
                return (Error,)+(None,)*8
            AllDSs[iDSName] = iDS
    Error = 0
    # 加载资金账户
    CashAccount = Account.CashAccount(dss=AllDSs,qs_env=qs_env)
    with shelve.open(StrategyPath+os.sep+"CashAccount") as File:
        Error += CashAccount.loadInfo(File)
    # 加载多头账户
    LongAccount = Account.LongAccount(dss=AllDSs,qs_env=qs_env)
    with shelve.open(StrategyPath+os.sep+"LongAccount") as File:
        Error += LongAccount.loadInfo(File)
    # 加载对冲账户
    ArbitrageAccount = Account.ArbitrageAccount(dss=AllDSs,qs_env=qs_env)
    with shelve.open(StrategyPath+os.sep+"ArbitrageAccount") as File:
        Error += ArbitrageAccount.loadInfo(File)
    # 加载空头账户
    ShortAccount = Account.ShortAccount(dss=AllDSs,qs_env=qs_env)
    with shelve.open(StrategyPath+os.sep+"ShortAccount") as File:
        Error += ShortAccount.loadInfo(File)
    if Error<4:
        return (-1,)+(None,)*8
    # 加载信号合并器
    Error = 0
    SignalMerger = SMClasses.SignalMerger(cash_account=CashAccount,long_account=LongAccount,arbitrage_account=ArbitrageAccount,short_account=ShortAccount,all_dss=AllDSs,qs_env=qs_env)
    with shelve.open(StrategyPath+os.sep+"SignalMerger") as File:
        Error += SignalMerger.loadInfo(File)
    # 加载信号生成器
    SGList = []
    SGNameList = []
    for iFileName in AllFiles:
        iSGClass = iFileName.split('-')
        if iSGClass[0] not in ("SG",'TSG'):
            continue
        iPos = int(iSGClass[1])
        with shelve.open(StrategyPath+os.sep+iFileName) as File:
            iSGType = File['Type']
            if iSGClass[0]=='SG':
                iSGGen = SGClasses.SGClasses.get(iSGType)
            elif iSGClass[0]=='TSG':
                iSGGen = TimingSGClasses.SGClasses.get(iSGType)
            else:
                iSGGen = None
            if iSGGen is None:
                qs_env.SysArgs["LastErrorMsg"] = '加载失败 : 该系统不支持此策略!'
                return (-1,)+(None,)*8
            else:
                iSG = iSGGen(cash_account=CashAccount,long_account=LongAccount,arbitrage_account=ArbitrageAccount,short_account=ShortAccount,all_dss=AllDSs,qs_env=qs_env)
            Error += iSG.loadInfo(File)
        iSGNum = len(SGList)
        if iSGNum<=iPos:
            SGList += [None]*(iPos-iSGNum+1)
            SGNameList += [None]*(iPos-iSGNum+1)
        SGList[iPos] = iSG
        SGNameList[iPos] = iSGClass[0]+'-'+iSGType
    SignalMerger.SGList = SGList
    SignalMerger.SGNameList = SGNameList
    # 加载信号处理器
    with shelve.open(StrategyPath+os.sep+"SignalProcessor") as File:
        SignalPrc = SPClasses.SPClasses[File['Type']](dss=AllDSs,cash_account=CashAccount,long_account=LongAccount,arbitrage_account=ArbitrageAccount,short_account=ShortAccount,qs_env=qs_env)
        Error += SignalPrc.loadInfo(File)
    # 加载结果集
    Result = Report.ResultReport(all_dss=AllDSs,cash_account=CashAccount,long_account=LongAccount,arbitrage_account=ArbitrageAccount,short_account=ShortAccount,signal_prc=SignalPrc,qs_env=qs_env)
    with shelve.open(StrategyPath+os.sep+"Result") as File:
        Error += Result.loadInfo(File)
    if Error<3+len(SGList):
        return (-1,)+(None,)*8
    return (1,AllDSs,CashAccount,LongAccount,ArbitrageAccount,ShortAccount,SignalMerger,SignalPrc,Result)

...
def expandListElementDataFrame(df, expand_index=True, dropna=False, empty_list_mask=False, len_col=None):
    ...
    if expand_index:
        ...
        # data = data.loc[:, Cols]
        data.iloc[:, :data.shape[1] - nCol] = (data.iloc[:, :data.shape[1] - nCol].map(lambda x: [x]).T * (ElementLen[Mask].values - 1)).T
...
def formatValue2MD(value, symbol_list=["*", "+", "-"], list_class=(list, np.ndarray), list_limit=5, dict_class=(dict, pd.Series), dict_limit=5, **kwargs):
    """格式化"""
    symbol_idx = kwargs.get("symbol_idx", 0)
    if isinstance(value, list_class):
        iNum = len(value)
        if iNum > list_limit:
            iLastNum = int(list_limit / 2)
            iFirstNum = list_limit - iLastNum
            return str(list(value[:iFirstNum]) + ["..."] + list(value[-iLastNum:])) + f", 共 {iNum} 个元素"
    elif isinstance(value, dict_class):
        iKeys = list(value.keys())
        iNum = len(value)
        if iNum > dict_limit:
        ...
        iMD = ''
        for ijKey in iKeys:
            ijVal = value.get(ijKey, "...")
            if isinstance(ijVal, dict_class):
                iMD += f"\n{'    ' * (symbol_idx + 1)}{symbol_list[symbol_idx % len(symbol_list)]} {str(ijKey)}: {formatValue2MD(ijVal, symbol_list=symbol_list, list_class=list_class, list_limit=list_limit, dict_class=dict_class, dict_limit=dict_limit, symbol_idx=symbol_idx+2)}"
            else:
                iMD += f"\n{'    ' * (symbol_idx + 1)}{symbol_list[symbol_idx % len(symbol_list)]} {str(ijKey)}: {str(ijVal)}"
        if not iMD: iMD = "{}"
        return f"共 {iNum} 个元素" + f": {{{iKeys[1:-1]}}}" if iNum > dict_limit else iMD
    ...
    if len(dict_like) > 0:
        ...
        for iKey, iVal in dict_like.items():
            ...
            if isinstance(iVal, list_class):
                ...
                MD.append(f"{'    t' * (symbol_idx)}{symbol_list[symbol_idx % len(symbol_list)]} {iKey}{f'(共 {iNum} 个元素)' if iNum > list_limit else ''}: {iVal}")
            elif isinstance(iVal, dict_class):
                iKeys = list(iVal.keys())
                iNum = len(iVal)
                if iNum > dict_limit:
                ...
                for ijKey in iKeys:
                    ...
                    if isinstance(ijVal, dict_class):
                        ...
                        iMD += f"\n{'    ' * (symbol_idx + 1)}{symbol_list[symbol_idx % len(symbol_list)]} {str(ijKey)}: {str(ijVal)}"
                MD.append(f"{'    ' * symbol_idx}{symbol_list[symbol_idx % len(symbol_list)]} {str(iKey)}{f'(共 {iNum} 个元素)' if iNum > dict_limit else ''}: {iMD if iMD else '{}'}")
            else:

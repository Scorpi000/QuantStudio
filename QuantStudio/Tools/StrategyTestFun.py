...
def backtestPortfolioStrategy_pd(portfolio: pd.DataFrame, price: pd.DataFrame) -> pd.Series:
    ...
    # 计算再平衡时点的净值
    RebalanceReturn = RebalancePrice / RebalancePrice.shift(1).bfill() - 1
    RebalanceNV = ((portfolio.shift(1) * RebalanceReturn).sum(axis=1) + 1).cumprod()
    # 计算所有时点的净值
    CostPrice = RebalancePrice.reindex(index=AllDTs).ffill().bfill()
    NV = (portfolio.reindex(index=AllDTs).ffill() * (AllPrice / CostPrice - 1)).sum(axis=1) + 1
...
# 格式化策略相关的统计指标
def formatStrategySummary(summary):
    ...
    FormattedStats.iloc[:2] = DateFormatFun(summary.iloc[:2, :].values)
    FormattedStats.iloc[2] = IntFormatFun(summary.iloc[2, :].values)
    FormattedStats.iloc[3:6] = PercentageFormatFun(summary.iloc[3:6, :].values)
    FormattedStats.iloc[6:8] = FloatFormatFun(summary.iloc[6:8, :].values)
    FormattedStats.iloc[8:10] = PercentageFormatFun(summary.iloc[8:10, :].values)
    FormattedStats.iloc[10:12] = DateFormatFun(summary.iloc[10:12, :].values)
...
def summaryTrade(trade_record, only_total=False):
    ...
    return Summary
# 格式化交易相关的统计指标
def formatTradeSummary(summary):
    FormattedStats = pd.DataFrame(index=summary.index, columns=summary.columns, dtype="O")
    DateFormatFun = np.vectorize(lambda x: x.strftime("%Y-%m-%d") if pd.notnull(x) else "NaT")
    IntFormatFun = np.vectorize(lambda x: ("%d" % (x, )))
    FloatFormatFun = np.vectorize(lambda x: ("%.2f" % (x, )))
    PercentageFormatFun = np.vectorize(lambda x: ("%.2f%%" % (x*100, )))
    FormattedStats.iloc[:3] = IntFormatFun(summary.iloc[:3, :].values)
    FormattedStats.iloc[3:4] = FloatFormatFun(summary.iloc[3:4, :].values)
    FormattedStats.iloc[4:11] = PercentageFormatFun(summary.iloc[4:11, :].values)
    FormattedStats.iloc[11:13] = FloatFormatFun(summary.iloc[11:13, :].values)
    return FormattedStats
def summarySingleTiming(start_idx, end_idx, ret, position_level, n_per_year=240):
    ...
    Summary["平均仓位水平"] = np.sum((end_idx - start_idx) * position_level) / Summary["持仓时长"]
    Summary["年化收益率"] = (Summary["累积收益率"] + 1)**(n_per_year / Summary["持仓时长"]) - 1
    Summary["单次平均收益率"] = np.mean(ret)
    Summary["平均盈利率"] = np.mean(ret[ProfitMask])
    Summary["平均亏损率"] = - np.mean(ret[~ProfitMask])
    Summary["盈亏比"] = Summary["平均盈利率"] / Summary["平均亏损率"]
    Summary["单次最大盈利率"]  = np.max(np.r_[0, ret])
    Summary["单次最大亏损率"]  = - np.min(np.r_[0, ret])
    ProfitIdx = np.diff(np.r_[0, ProfitMask.astype(float), 0])
    Summary["最大连续盈利次数"] = np.max(np.r_[0, np.where(ProfitIdx==-1)[0] - np.where(ProfitIdx==1)[0]])
...
def summaryTimingStrategy(position, price, ignore_position_level=True, n_per_year=240):
    ...
    position[0, np.isnan(position[0])] = 0
    PositionNafilled = pd.DataFrame(position).fillna(method="pad").values
    price = np.r_[price, price[-1:]]
    LongIdx = np.diff(np.r_[np.zeros((1, nID)), (PositionNafilled>0).astype(float)], axis=0)
    ShortIdx = np.diff(np.r_[np.zeros((1, nID)), (PositionNafilled<0).astype(float)], axis=0)
    LongSummary = pd.DataFrame(np.nan, index=["择时次数", "正确次数", "错误次数", "正确率", "累积收益率", "年化收益率", "单次平均收益率", 
                                              "平均盈利率", "平均亏损率", "盈亏比", "单次最大盈利率", "单次最大亏损率", "最大连续盈利次数", "最大连续亏损次数", 
                                              "持仓时长", "平均持仓时长", "平均仓位水平"], columns=np.arange(nID))
...
# 格式化择时策略相关的统计指标
def formatTimingStrategySummary(summary):
    FormattedStats = pd.DataFrame(index=summary.index, columns=summary.columns, dtype="O")
    DateFormatFun = np.vectorize(lambda x: x.strftime("%Y-%m-%d") if pd.notnull(x) else "NaT")
    IntFormatFun = np.vectorize(lambda x: ("%d" % (x, )))
    FloatFormatFun = np.vectorize(lambda x: ("%.2f" % (x, )))
    PercentageFormatFun = np.vectorize(lambda x: ("%.2f%%" % (x*100, )))
    FormattedStats.iloc[:3] = IntFormatFun(summary.iloc[:3, :].values)
    FormattedStats.iloc[3:9] = PercentageFormatFun(summary.iloc[3:9, :].values)
    FormattedStats.iloc[9] = FloatFormatFun(summary.iloc[9, :].values)
    FormattedStats.iloc[10:12] = PercentageFormatFun(summary.iloc[10:12, :].values)
    FormattedStats.iloc[12:15] = IntFormatFun(summary.iloc[12:15, :].values)

...
class _FactorTable(FactorTable):
    ...
    def __init__(self, name, fdb, sys_args={}, **kwargs):
        self._Lock = fdb._Lock
        self._Data = fdb._Data
        self._TableMeta = fdb._TableMeta
        self._FactorMeta = fdb._FactorMeta
    ...
    def getMetaData(self, key=None, args={}):
        with self._Lock.
            ...
            if key is None:
                return pd.Series(self._TableMeta[self._Name])
            else:
                return self._TableMeta[self._Name].get(key, None)
    ...
    def getFactorMetaData(self, factor_names=None, key=None, args={}):
        ...
        MetaData = {}
        with self._Lock:
            FactorMeta = self._FactorMeta.get(self._Name, {})
            for iFactorName in factor_names:
                if iFactorName in FactorMeta:
                    if key is None:
                        MetaData[iFactorName] = FactorMeta.get(iFactorName, {})
                    else:
                        MetaData[iFactorName] = FactorMeta.get(iFactorName, {}).get(key, None)
    ...
    def getID(self, ifactor_name=None, idt=None, args={}):
        with self._Lock:
            ...
            if idt is None:
                return sorted(self._Data[self._Name][ifactor_name].columns)
            elif idt not in self._Data[self._Name][ifactor_name].index:
                return []
            else:
                return sorted(self._Data[self._Name][ifactor_name].loc[idt].dropna().index)
    
    def getDateTime(self, ifactor_name=None, iid=None, start_dt=None, end_dt=None, args={}):
        with self._Lock:
            if ifactor_name is None: ifactor_name = self.FactorNames[0]
            if iid is None:
                DTs = self._Data[self._Name][ifactor_name].index
            elif iid not in self._Data[self._Name][ifactor_name].columns:
                return []
            else:
                DTs = self._Data[self._Name][ifactor_name].loc[:, iid].dropna().index
    ...
    def _readFactorData(self, ifactor_name, ids, dts, args={}):
        with self._Lock:
            return self._Data[self._Name][ifactor_name].reindex(index=dts, columns=ids)
...
class MemoryDB(WritableFactorDB):
    ...
    def setTableMetaData(self, table_name, key=None, value=None, meta_data=None):
        with self._Lock:
            ...
            if table_name not in self._TableMeta:
                ...
                iDict = self._TableMeta[table_name]
                iDict.update(meta_data)
                self._TableMeta[table_name] = iDict
        return 0
    ...
    def renameFactor(self, table_name, old_factor_name, new_factor_name):
        ...
        with self._Lock:
            ...
            iDict[new_factor_name] = iDict.pop(old_factor_name)
            self._Data[table_name] = iDict
            if (table_name in self._FactorMeta) and (old_factor_name in self._FactorMeta[table_name]):
                iDict = self._FactorMeta[table_name]
                iDict[new_factor_name] = iDict.pop(old_factor_name)
                self._FactorMeta[table_name] = iDict
        return 0
    ...
    def setFactorMetaData(self, table_name, ifactor_name, key=None, value=None, meta_data=None):
        with self._Lock:
            Meta = self._FactorMeta.get(table_name, {})
            iMeta = Meta.setdefault(ifactor_name, {})
            if key is not None:
                iMeta[key] = value
            if meta_data is not None:
                for iKey in meta_data:
                    iMeta[iKey] = meta_data[iKey]
            self._FactorMeta[table_name] = Meta
        return 0

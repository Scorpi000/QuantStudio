        ...
        for iField, iRawData in raw_data.items():
            if isinstance(iRawData, pd.DataFrame) and (id_col in iRawData):  # 如果原始数据有 ID 列，按照 ID 列划分后存入子进程的原始文件中
                ...
                for jPID, jIDs in pid_ids.items():
                    ...
                    jPath = self._RawDataDir + os.sep + jPID + os.sep + key + self._QSArgs.Suffix
                    with self._DataLock:
                        self.writeDataFrame(path=jPath, target_field=iField, data=ijRawData.reset_index(), if_exists=if_exists, ignore_index=True)
            else:  # 如果原始数据没有 ID 列，则将所有数据分别存入子进程的原始文件中
                for jPID, jIDs in pid_ids.items():
                    jPath = self._RawDataDir + os.sep + jPID + os.sep + key + self._QSArgs.Suffix
                    with self._DataLock:
                        self.writeDataFrame(jPath, iRawData, target_field=None, if_exists=if_exists, ignore_index=True)
    ...
    def readRawData(self, key, target_fields=None, pids=None):
        ...
        for iPID in pids:
            ...
            with self._PIDLock[iPID]:
                iRawData = self.readDataFrame(iRawDataPath, target_fields=target_fields)
            for jField, jVal in iRawData.items(): RawData.setdefault(jField, []).append(jVal)
...
class FeatherCache(FileCache):
    ...
    def readDataFrame(self, path: str, target_fields: Optional[list[str]]=None, data_type=None):
        ...
        for iField in target_fields:
            ...
            if os.path.isfile(iPath):
                RawData[iField] = pd.read_feather(iPath)
        return RawData


class HDF5Cache(FileCache):
    class __QS_ArgClass__(FileCache.__QS_ArgClass__):
        Suffix: str = Field(default=".h5", title="后缀", frozen=True)
    
    def createPath(self, path: str):
        with pd.HDFStore(path, mode="a") as iFile: pass
    ...
    def writeDataFrame(self, path: str, target_field: str, data: pd.DataFrame, if_exists: Literal["append", "replace"]="replace", ignore_index=True):
        with pd.HDFStore(path, mode="a") as File:
            if (target_field in File) and (if_exists == "append"):
                OldData = File[target_field]
                Data = pd.concat([OldData, data], ignore_index=ignore_index)
                if not ignore_index: Data = Data[~Data.index.duplicated()]
                File[target_field] = Data.sort_index()
            else:
                File[target_field] = data

    def readDataFrame(self, path: str, target_fields: Optional[list[str]]=None):
        if not os.path.isfile(path): return {}
        RawData = {}
        with pd.HDFStore(path, mode="r") as File:
            target_fields = (sorted(jField[1:] for jField in File) if target_fields is None else target_fields)
            for jField in target_fields:
                if jField in File:
                    RawData[jField] = File[jField]
        return RawData
        ...
        for iField, iRawData in raw_data.items():
            if isinstance(iRawData, pd.DataFrame) and (id_col in iRawData):  # 如果原始数据有 ID 列，按照 ID 列划分后存入子进程的原始文件中
                ...
                for jPID, jIDs in pid_ids.items():
                    jInterIDs = sorted(AllIDs.intersection(jIDs))
                    ijRawData = iRawData.loc[jInterIDs]
                    jFilePath = self._RawDataDir + os.sep + jPID + os.sep + key + self._QSArgs.Suffix
                    with self._DataLock:
    ...
    def readRawData(self, key, target_fields=None, pids=None):
        ...
        RawData = {}
        for iPID in pids:
            iRawDataFilePath = self._RawDataDir + os.sep + iPID + os.sep + key + self._QSArgs.Suffix
            with self._PIDLock[iPID]:
    ...
    def checkFactorDataExistence(self, key, pids=None):
        ...
        with self._DataLock:
            for iPID in pids:
                iFilePath = self._FactorDataDir + os.sep + iPID + os.sep + key + self._QSArgs.Suffix
        ...
        for iPID in PIDs:
            with self._PIDLock[iPID]:
                if pid_ids is not None:
                    ...
                    if not os.path.isfile(iFilePath): 
                        with open(iFilePath, mode="a") as iFile: pass
                    with pd.HDFStore(self._FactorDataDir + os.sep + iPID + os.sep + key + self._QSArgs.Suffix, mode="a") as CacheFile:
                        if (target_field in CacheFile) and (if_exists == "append"):
    ...
    def readFactorData(self, key, ipid, target_field="StdData", pids=None, wait=True, wait_seconds=0.1):
        if isinstance(pids, str):
            FilePath = self._FactorDataDir + os.sep + pids + os.sep + key + self._QSArgs.Suffix

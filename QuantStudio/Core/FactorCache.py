...
class FileCache(FactorCache):
    class __QS_ArgClass__(FactorCache.__QS_ArgClass__):
        ...
        FileSuffix: str = Field(default="", title="文件后缀", frozen=True)
    ...
    def writeDataFrame(self, path: str, data: pd.DataFrame, if_exists: Literal["append", "replace"]="replace", ignore_index=True):
        # with pd.HDFStore(jFilePath, mode="a") as jFile:
        ...
        # with pd.HDFStore(iFilePath, mode="a") as CacheFile:
    ...
    def readDataFrame(self, path: str):
    ...
    def checkRawDataExistence(self, key, pids=None, if_not_exists="create"):
        ...
        with self._DataLock:
            for iPID in pids:
                iFilePath = self._RawDataDir + os.sep + iPID + os.sep + key + self._QSArgs.FileSuffix
                if os.path.isfile(iFilePath):
                    IfExist = True
                    if if_not_exists != "create":
                        break
                elif if_not_exists == "create":
                    with open(iFilePath, mode="a") as iFile: pass
        return IfExist
        ...
        for iField, iRawData in raw_data.items():
            if isinstance(iRawData, pd.DataFrame) and (id_col in iRawData):  # 如果原始数据有 ID 列，按照 ID 列划分后存入子进程的原始文件中
                ...
                for jPID, jIDs in pid_ids.items():
                    ...
                    jFilePath = self._RawDataDir + os.sep + jPID + os.sep + key + self._QSArgs.FileSuffix
                    with self._DataLock:
                        self.writeDataFrame(jFilePath, ijRawData.reset_index(), target_field=iField, if_exists=if_exists, ignore_index=True)
            else:  # 如果原始数据没有 ID 列，则将所有数据分别存入子进程的原始文件中
                for jPID, jIDs in pid_ids.items():
                    jFilePath = self._RawDataDir + os.sep + jPID + os.sep + key + self._QSArgs.FileSuffix
                    with self._DataLock:
                        self.writeDataFrame(jFilePath, iRawData, target_field=None, if_exists=if_exists, ignore_index=True)

    def readRawData(self, key, target_fields=None, pids=None):
        if pids is None: pids = self._QSArgs.PIDs
        RawData = {}
        for iPID in pids:
            iRawDataFilePath = self._RawDataDir + os.sep + iPID + os.sep + key + self._QSArgs.FileSuffix
            with self._PIDLock[iPID]:
                if os.path.isfile(iRawDataFilePath):
                    RawData = self.readDataFrame(iRawDataFilePath)
    ...
    def checkFactorDataExistence(self, key, pids=None):
        ...
        with self._DataLock:
            for iPID in pids:
                iFilePath = self._FactorDataDir + os.sep + iPID + os.sep + key + self._QSArgs.FileSuffix
                IfExist = os.path.isfile(iFilePath) or IfExist
        return IfExist
        ...
        for iPID in PIDs:
            with self._PIDLock[iPID]:
                if pid_ids is not None:
                    ...
                    iFilePath = self._FactorDataDir + os.sep + iPID + os.sep + key + self._QSArgs.FileSuffix
                    self.writeDataFrame(iFilePath, factor_data.reindex(columns=iIDs), if_exists=if_exists)
                else:
                    iFilePath = self._FactorDataDir + os.sep + iPID + os.sep + key + self._QSArgs.FileSuffix
                    self.writeDataFrame(iFilePath, factor_data, if_exists=if_exists)

    def readFactorData(self, key, ipid, target_field="StdData", pids=None, wait=True, wait_seconds=0.1):
        if isinstance(pids, str):
            FilePath = self._FactorDataDir + os.sep + pids + os.sep + key + self._QSArgs.FileSuffix
            if not os.path.isfile(FilePath):
                return None
            with self._PIDLock[pids]:
                FactorData = self.readDataFrame(FilePath)
            return FactorData
        iFilePath = self._FactorDataDir + os.sep + ipid + os.sep + key + self._QSArgs.FileSuffix
        with self._PIDLock[ipid]:
            DTNum = self.readDataFrame(iFilePath).shape[0]
        if pids is None:
            pids = {ipid}
        else:
            pids = set(pids)
        ...
        while len(pids) > 0:
            ...
            if not os.path.isfile(iFilePath):  # 该进程的数据没有准备好
                ...
                if (iPID not in MTime) or (iMTime > MTime[iPID]):
                    MTime[iPID] = iMTime
                    iDTNum = self.readDataFrame(iFilePath).shape[0]
                    if iDTNum != DTNum:
                        pids.add(iPID)
                        if wait_seconds > 0: time.sleep(wait_seconds)
                        continue
...
class FeatherCache(FileCache):
    class __QS_ArgClass__(FileCache.__QS_ArgClass__):
        FileSuffix: str = Field(default=".feather", title="文件后缀", frozen=True)
    
    def writeDataFrame(self, path: str, data: pd.DataFrame, target_field=None, if_exists: Literal["append", "replace"]="replace", ignore_index=True):
        if target_field: path = path[:-len(self._QSArgs.FileSuffix)] + "-" + target_field + self._QSArgs.FileSuffix
        if (if_exists=="append") and os.path.isfile(path):
            ...
            Data.sort_index().to_feather(path)
        else:
            data.to_feather(path)

    def readDataFrame(self, path: str, target_fields=None):
        Prefix = path[:-len(self._QSArgs.FileSuffix)]
        if not target_fields:
            target_fields = [iFile.split("-")[-1] for iFile in os.listdir(os.path.split(path)[0]) if iFile.startswith(Prefix)]
        RawData = {}
        for iField in target_fields:
            iPath = Prefix + "-" + iField + self._QSArgs.FileSuffix
            RawData.setdefault(iField, []).append(pd.read_feather(iPath))
        ...
        with self._DataLock:
            for iPID in pids:
                ...
                if os.path.isfile(iFilePath):
                    ...
                    with open(iFilePath, mode="a") as iFile: pass
                    # with pd.HDFStore(iFilePath, mode="a") as iFile: pass
        return IfExist
        ...
        for iField, iRawData in raw_data.items():
            if isinstance(iRawData, pd.DataFrame) and (id_col in iRawData):  # 如果原始数据有 ID 列，按照 ID 列划分后存入子进程的原始文件中
                ...
                for jPID, jIDs in pid_ids.items():
                    jInterIDs = sorted(AllIDs.intersection(jIDs))
                    ijRawData = iRawData.loc[jInterIDs]
                    jFilePath = self._RawDataDir + os.sep + jPID + os.sep + key + self._QSArgs.HDF5Suffix
                    with self._DataLock:
                        if not os.path.isfile(jFilePath):
                            with open(jFilePath, mode="a") as jFile: pass
                        with pd.HDFStore(jFilePath, mode="a") as jFile:
                            if (iField in jFile) and (if_exists == "append"):
                                jFile[iField] = pd.concat([jFile[iField], ijRawData.reset_index()], ignore_index=True)
                            else:
    ...
    def checkFactorDataExistence(self, key, pids=None):
        ...
        with self._DataLock:
            for iPID in pids:
                ...
                with pd.HDFStore(iFilePath, mode="r") as iFile:
        ...
        for iPID in PIDs:
            with self._PIDLock[iPID]:
                if pid_ids is not None:
                    ...
                    if not os.path.isfile(iFilePath): 
                        with open(iFilePath, mode="a") as iFile: pass
                    with pd.HDFStore(self._FactorDataDir + os.sep + iPID + os.sep + key + self._QSArgs.HDF5Suffix, mode="a") as CacheFile:
                        if (target_field in CacheFile) and (if_exists == "append"):

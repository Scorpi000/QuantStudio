    ...
    def backward_compute(self, path: List[str], bwd_data_list: List[Any], context: Context, local_context: Any=None) -> Any:
        if hasattr(self._QSArgs.TargetFDB, "writeFactorData"):
            for i, iData in enumerate(bwd_data_list):
                self._QSArgs.TargetFDB.writeFactorData(factor_data=iData, table_name=self._QSArgs.TargetTable, ifactor_name=self.Deps[i]._QSArgs.Name, if_exists=self._QSArgs.IfExists)
        else:
            Data = Panel({self.Deps[i]._QSArgs.Name: iData for i, iData in enumerate(bwd_data_list)})
            self._QSArgs.TargetFDB.writeData(data=Data, table_name=self._QSArgs.TargetTable, if_exists=self._QSArgs.IfExists)

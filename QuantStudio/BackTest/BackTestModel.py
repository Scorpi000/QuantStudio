...
class ReportNode(BTNode):
    """报告生成节点基类, 依赖计算节点, 在结果集中添加报告"""

    class __QS_ArgClass__(BTNode.__QS_ArgClass__):
        Name: str = Field(default="ReportNode", frozen=True, title="名称")
        ReportKey: str = Field(default="Report", frozen=True, title="报告键名", description="在输出结果集中存储报告的键名")
...
class BTReport(ReportNode):
    ...
    def __init__(self, report_nodes: List[ReportNode] = [], args: dict = {}, config_file: Optional[str] = None, **kwargs):
        return super().__init__(deps=report_nodes, args=args, config_file=config_file, **kwargs)
    ...
    @staticmethod
    def genOutputReport(output_list:List[dict], name_list:Optional[List[str]]=None, report_key:str="Report") -> str:
        if not name_list: name_list = [""] * len(output_list)
        HTML = ""
        SepStr = '<HR style="FILTER: alpha(opacity=100,finishopacity=0,style=3)" width="90%" color=#987cb9 SIZE=5><div align="center" style="font-size:1.17em"><strong>{Module}</strong></div>'
        for i, iOutput in enumerate(output_list):
            if report_key in iOutput:
                iHTML = iOutput[report_key]
            else:
                iHTML = "暂无报告"

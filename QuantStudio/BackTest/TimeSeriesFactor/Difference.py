...
class QuantileDifference(BaseModule):
    ...
    def __QS_move__(self, idt, **kwargs):
        ...
        FactorData = self._FactorTable.readData(dts=[PreDateTime], ids=self._Output["证券ID"], factor_names=[self.TestFactor]).iloc[0, 0, :].values
        ...
        for j in range(self.GroupNum):
            if j==0: Mask[j] = (FactorData<=np.percentile(FactorData, (j+1)/self.GroupNum*100, axis=0))
            else: Mask[j] = ((FactorData>np.percentile(FactorData, j/self.GroupNum*100, axis=0)) & (FactorData<=np.percentile(FactorData, (j+1)/self.GroupNum*100, axis=0)))

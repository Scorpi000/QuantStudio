...
class TimingStrategy(Strategy):
    ...
    def TargetIDs(self):# 当前有分配资金的 ID 列表
        if self._ValueAllocated is None:
            if self._QSArgs.ValueAllocated is None:
                if self._QSArgs.TargetAccount is not None: return self._QSArgs.TargetAccount.IDs
                else: return []
            return self._QSArgs.ValueAllocated.index.tolist()
        else:
            return self._ValueAllocated[self._ValueAllocated!=0].index.tolist()
    @property
    def PositionLevel(self):# 当前目标账户中所有 ID 的仓位水平
        if self._QSArgs.TargetAccount is None: raise __QS_Error__("尚未设置目标账户!")
        if self._CashAllocated is None: return pd.Series(0.0, index=self._QSArgs.TargetAccount.IDs)
    ...
    def _resetAllocation(self, new_allocation):
        IDs = self._QSArgs.TargetAccount.IDs
        if new_allocation is None:
            return pd.Series(self._QSArgs.TargetAccount.AccountValue / len(IDs), index=IDs)
    ...
    def __QS_start__(self, mdl, dts, **kwargs):
        ...
        self._ValueAllocated = self._resetAllocation(self._QSArgs.ValueAllocated)
        self._CashAllocated = self._ValueAllocated - self._QSArgs.TargetAccount.PositionAmount.fillna(0.0)
    ...
    def __QS_move__(self, idt, **kwargs):
        ...
        if (not self._QSArgs.SigalDTs) or (idt in self._QSArgs.SigalDTs):
        ...
        if self._isAllocationReseted:
            NewAllocation = self._resetAllocation(self._QSArgs.ValueAllocated)
        ...
        if NewAllocation is not None:
            ...
            iTradingRecord = TradingRecord[self._QSArgs.TargetAccount.Name]
    ...
    def trade(self, idt, trading_record, signal):
        ...
        if signal is not None:# 有新的信号, 形成新的交易目标
            ...
            elif self._QSArgs.TradeTarget=="锁定目标仓位":
            ...
            if self._SignalExcutePeriod>=self._QSArgs.SigalValidity:
    ...
    def _bufferAllocationReset(self, allocation):
        if self._QSArgs.SignalDelay<=0: return allocation
        ...
        while self._TempData['StoredAllocation']!=[]:
            if self._TempData['AllocationLagNum'][0]>=self._QSArgs.SignalDelay:

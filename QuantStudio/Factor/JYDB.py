...
def _importInfo(info_file, info_resource, logger, out_info=False):
    ...
    if Suffix in ("xlsx", "xls"):
        ...
        ExchangeInfo = pd.read_excel(info_resource, "ExchangeInfo", dtype={"ExchangeCode": "O"},
                                     engine="openpyxl").set_index(["ExchangeCode"])
        SecurityInfo = pd.read_excel(info_resource, "SecurityInfo", dtype={"SecurityCategoryCode": "O"},
                                     engine="openpyxl").set_index(["SecurityCategoryCode"])
    elif Suffix == "json":
        ...
        FactorInfo = pd.concat([pd.DataFrame(Info["FactorInfo"][iTableName]).T for iTableName in TableNames],
                               keys=TableNames)
    else:
    ...
    if not os.path.isfile(info_file):
        ...
        try:
            ...
            return (readNestedDictFromHDF5(info_file, ref="/TableInfo"),
                    ...
                    readNestedDictFromHDF5(info_file, ref="/ExchangeInfo"),
                    readNestedDictFromHDF5(info_file, ref="/SecurityInfo"))
        except:
...
def deSuffixID(ids, sep='.'):
    return [(sep.join(iID.split(sep)[:-1]) if iID.find(sep) != -1 else iID) for iID in ids]


# 根据字段的数据类型确定 QS 的数据类型
def _identifyDataType(field_data_type):
    field_data_type = field_data_type.lower()
    if (field_data_type.find("num") != -1) or (field_data_type.find("int") != -1) or (
            field_data_type.find("decimal") != -1) or (field_data_type.find("double") != -1) or (
            field_data_type.find("float") != -1):
        return "double"
            ...
            if isinstance(iInfo, str) and (iInfo.find("从表") != -1):
                ...
                if iSETable not in SETables:
                    if iTypeCode is None:
                        JoinStr.append(
                            "LEFT JOIN " + iSETable + " ON (" + self._DBTableName + ".ID=" + iSETable + "." + iJoinField + ")")
                    else:
                        JoinStr.append(
                            "LEFT JOIN " + iSETable + " ON (" + self._DBTableName + ".ID=" + iSETable + "." + iJoinField + " AND " + iSETable + ".TypeCode=" + iTypeCode + ")")
...
def genANN_ReportSQLStr(db_type, table_prefix, ids, report_period="1231", pre_filter_id=True):
    ...
    if pre_filter_id:
        SQLStr += "AND (" + genSQLInCondition(table_prefix + "SecuMain.SecuCode", deSuffixID(ids), is_str=True,
                                              max_num=1000) + ") "
    else:
...
def _prepareReportANNRawData(fdb, ft, ids, pre_filter_id=True, args={}):
    SQLStr = genANN_ReportSQLStr(fdb._QSArgs.DBType, fdb._QSArgs.TablePrefix, ids, report_period="1231",
                                 pre_filter_id=pre_filter_id)
    RawData = fdb.fetchall(SQLStr)
    if not RawData:
        RawData = pd.DataFrame(columns=["ID", "公告日期", "报告期"])
    else:
        RawData = pd.DataFrame(np.array(RawData, dtype="O"), columns=["ID", "公告日期", "报告期"])
    ...
    def __QS_calcData__(self, raw_data, factor_names, ids, dts, args={}):
        ...
        if CalcType == "Fwd12M":
            ...
            if (ANNReportPath is not None) and os.path.isfile(ANNReportPath):
                ...
                ANNReportData = _prepareReportANNRawData(self._FactorDB, self, ids,
                                                         pre_filter_id=args.get("预筛选ID", self._QSArgs.PreFilterID),
                                                         args=args)
            ...
            for i, iDate in enumerate(date_seq):
                ...
                for j in range(0, tempInd + 1):
                    if kRawData['报告期'].iloc[tempInd - j] == ObjectDate:
                        ...
                        if (iDate - dt.date(int(FYNoteDate[0:4]), int(FYNoteDate[4:6]),
                                            int(FYNoteDate[6:]))).days <= lookback:
            ...
            for i, iDate in enumerate(date_seq):
                ...
                for j in range(0, tempInd + 1):
                    if (ObjectData1 is None) and (kRawData['报告期'].iloc[tempInd - j] == ObjectDate1):
                        ...
                        if (iDate - dt.date(int(NoteDate[0:4]), int(NoteDate[4:6]),
                                            int(NoteDate[6:]))).days <= lookback:
                ...
                if (ObjectData1 is not None) and (ObjectData2 is not None):
                    ...
                    if (iDate.month == 2) and (iDate.day == 29):
                        Weight1 = Weight1 / 366
                    else:
...
class _AnalystEstDetailTable(_JY_SQL_Table):
    ...
    def __QS_prepareRawData__(self, factor_names, ids, dts, args={}):
        ...
        RawData = self._FactorDB.fetchall(SQLStr)
        if not RawData:
            RawData = pd.DataFrame(columns=["日期", "ID", self._ReportDateField] + AllFields)
        else:
            RawData = pd.DataFrame(np.array(RawData, dtype="O"),
                                   columns=["日期", "ID", self._ReportDateField] + AllFields)
    ...
    def __QS_calcData__(self, raw_data, factor_names, ids, dts, args={}):
        ...
        for kFactorName in factor_names:
            if DataType == "double":
                kData = np.full(shape=(len(Dates), len(ids)), fill_value=np.nan)
            else:
                kData = np.full(shape=(len(Dates), len(ids)), fill_value=None, dtype="O")
            ...
            for j, jID in enumerate(ids):
                ...
                for i, iDate in enumerate(Dates):
                    ...
                    ijRawData = jRawData[
                        (jRawData["日期"] <= iDate.strftime("%Y%m%d")) & (jRawData["日期"] > iStartDate)]
                    iLastYear = str(iDate.year - 1)
                    ijNoteDate = findNoteDate(iLastYear + "1231", jReportNoteDate)
                    x = []
                    for iiNFY in ForwardYears:
                        if (ijNoteDate is None) or (
                                (ijNoteDate is not None) and (ijNoteDate > iDate.strftime("%Y%m%d"))):
                        ...
                        if iijRawData.shape[0] == 0:
                            ...
                            if DeduplicationFields:
                                ...
                                iijRawData = pd.merge(ijTemp, iijRawData, how="left",
                                                      left_on=DeduplicationFields + ["日期"],
...
class _AnalystRatingDetailTable(_JY_SQL_Table):
    ...
    def __QS_prepareRawData__(self, factor_names, ids, dts, args={}):
        ...
        RawData = self._FactorDB.fetchall(SQLStr)
        if not RawData:
            return pd.DataFrame(columns=["日期", "ID"] + AllFields)
        else:
            RawData = pd.DataFrame(np.array(RawData, dtype="O"), columns=["日期", "ID"] + AllFields)
        if self._FactorDB._QSArgs.DBType not in ("SQL Server", "MySQL", "Oracle"):
            RawData["日期"] = self.__QS_adjustDT__(RawData["日期"])
            RawData["日期"] = RawData["日期"].dt.strftime("%Y%m%d")
...
class JYDB(QSSQLObject, FactorDB):
    ...
    def getTradeDay(self, start_date:Optional[dt.datetime]=None, end_date:Optional[dt.datetime]=None, exchange:Literal["SSE", "SZSE", "SHFE", "DCE", "CZCE", "INE", "CFFEX"]="SSE", **kwargs) -> List[dt.datetime]:
        ...
        if ExchangeCode.shape[0] == 0:
            raise __QS_Error__("不支持的交易所: %s" % exchange)
        else:
    ...
    def getBondID(self, exchange:Optional[Union[str, Tuple[str]]]=None, date:Optional[dt.datetime]=None, is_current:bool=True, start_date:Optional[dt.datetime]=None, **kwargs) -> List[str]:
        ...
        try:
            ...
            self._QS_Logger.warning(
                "使用债券代码对照表(Bond_Code)提取债券 ID 失败: %s, 将使用证券主表(SecuMain)提取债券 ID, 将忽略参数 date, is_current, start_date!" % (
                    str(e),))
    ...
    def getFutureID(self, exchange:Optional[Union[str, Tuple[str]]]=None, future_code:Optional[Union[str, Tuple[str]]]=None, date:Optional[dt.datetime]=None, is_current:bool=True, start_date:Optional[dt.datetime]=None, **kwargs) -> List[str]:
        ...
        if future_code:
            if isinstance(future_code, str):
                ...
                SQLStr += "AND (SUBSTRING(ContractCode, 1, 1) IN ('" + "','".join(
                    future_code) + "') OR SUBSTRING(ContractCode, 1, 2) IN ('" + "','".join(future_code) + "')) "
        ...
        if ContractType != "连续合约":
            ...
            if isinstance(ContinueContractType, str):
                ...
                SubSQLStr = "SELECT DM FROM {Prefix}CT_SystemConst WHERE LB = 2352 AND MS IN (" + "', '".join(
                    ContinueContractType) + "')"
    ...
    def getIndustryID(self, standard:str="中信行业分类", level:int=1, date:Optional[dt.datetime]=None, is_current:bool=True, start_date:Optional[dt.datetime]=None, **kwargs) -> List[str]:
        ...
        if len(Standard) != 1:
            ...
            raise __QS_Error__("无法识别的行业分类标准 : %s, 支持的行业分类标准有 : %s" % (standard, ", ".join(
                iStandard[0] for iStandard in AllStandards)))
        if date is None: date = dt.date.today()
        if start_date is not None: start_date = start_date.strftime("%Y-%m-%d %H:%M:%S")

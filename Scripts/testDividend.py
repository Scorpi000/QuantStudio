...
MarketData = MarketFT.readData(factor_names=["昨收盘价", "收盘价"], ids=IDs, dts=DateTimes)
DvdData = WDB.getTable("中国A股分红").readData(factor_names=["每股派息(税前)", "每股送转"], ids=IDs, dts=DateTimes)
DvdData.fillna(0, inplace=True)

RightIssueData = WDB.getTable("中国A股配股").readData(factor_names=["配股价格", "配股比例"], ids=IDs, dts=DateTimes)
